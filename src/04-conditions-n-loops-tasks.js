...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const sortedValues = a < b ? `${a}, ${b}` : `${b}, ${a}`;
  return `${leftBracket}${sortedValues}${rightBracket}`;
}
...
function isCreditCardNumber(ccn) {
  const digitsArray = [...`${ccn}`];
  const reversedDigitsArray = digitsArray.reverse();
  const parsedDigitsArray = reversedDigitsArray.map((num) => +num);

  const sum = parsedDigitsArray.reduce((accumulator, currentDigit, idx) => {
    if (idx % 2 === 0) {
      return accumulator + currentDigit;
    }
    const multipliedDigit = currentDigit * 2;
    const toSum = multipliedDigit > 9 ? multipliedDigit - 9 : multipliedDigit;
...
function getDigitalRoot(num) {
  const sum = [...String(num)].reduce((a, b) => +a + +b);
...
function isBracketsBalanced(str) {
  ...
  return newStr === str ? str.length === 0 : isBracketsBalanced(newStr);
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathes.length; i += 1) {
    ...
    splitPaths.push(temp);
  }
  for (let i = 0; i < splitPaths[0].length; i += 1) {
    const tempArr = [];
    for (let k = 0; k < splitPaths.length; k += 1) {
      if (splitPaths[0][i] === splitPaths[k][i]) {
        tempArr.push(splitPaths[0][i]);
      }
    }
    if (tempArr.length === pathes.length) {
      commonPath.push(splitPaths[0][i]);
    } else {
      break;
    }
  }
